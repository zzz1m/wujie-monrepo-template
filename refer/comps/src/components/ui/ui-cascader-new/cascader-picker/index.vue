<template>
  <div :class="['cascader-viewer', filterable ? 'filterable' : '', className, showPicker && 'show-picker']" :style="styles">
    <!-- 过滤支持选择 -->
    <ui-select
      v-if="filterable"
      ref="filter"
      v-model="checkedDicts"
      class="filterabl-input"
      clearable
      filterable
      placeholder="请输入关键字"
      remote
    ...
    >
      <template v-if="fullDictList && fullDictList.length > 0">
        <ui-option
          v-for="item in fullDictList"
          :key="item.dictCodeFullPath"
          :class="{ active: filterChecked(item.dictCodeFullPath) }"
          :value="item.dictCodeFullPath"
          @click.native="changeDict(item)"
          >{{ item.dictNameFullPath }}</ui-option
        >
      </template>
    </ui-select>

    <cascader-node v-if="!isEmpty && showPicker" :node="root" :style="{ display: showDictSelector ? 'block' : 'none' }"></cascader-node>
...
const { _dictSearchByKeyword } = commonServices;
...
export default {
  ...
  methods: {
    ...
    resetStore() {
      const { nodeKey, data, props, onlyLeaf, lazy, load, multiple, stage, defaultCheckedKeys, allowCrossLevel } = this;
