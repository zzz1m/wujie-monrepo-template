<template>
  <div v-clickoutsider="hidePicker" class="ui-cascader-new">
    <div ref="reference" class="ui-cascader-reference">
      <!-- 输入框 -->
      <ui-input
        v-if="!limited"
        v-model="checkedVal"
        :class="['ctrl-input', showPicker ? 'open' : '']"
      ...
      >
        <i
          ...
          slot="suffix"
          :class="['icon-arrow', clearable && checkedVal ? 'ui-icon-circle-close' : 'ui-icon-arrow-down']"
          @click.stop="clear"
        ></i>
      </ui-input>
      <ui-tooltip v-if="limited" :content="checkedVal" effect="dark" placement="right-end" popper-class="ui-cascader-new-tooltip">
        <ui-input
...
import { util, common, emitter } from '@noah/tools';
import { remove } from '../../../utils/dom';
import { deepCopy } from '../../../utils/object';
const { calcStrLength } = common;
const { oneOf } = util;
...
const filterMetaData = function (metaData = [], store = {}) {
  ...
  function getNodeByLabel(label) {
    return Object.values(nodesMap).find((node) => {
      return node.label === label || node.data.dictNameFullPath === label || node.data.dictNameFullPath?.endsWith(label);
    });
  }
};
