    ...
    >
      <div :class="['upload-box', { 'is-uploaded': value }]" @click="mockClick">
        ...
        <div v-else class="avatar">
          <img v-if="downLoadUrl" alt="" :src="imgUrl || iconUrl" />
          <div v-else class="avatar-bg" :style="{ backgroundImage: `url('${relativePath ? getImgPreviewPath(value) : value}')` }"></div>
        </div>
        <div v-if="!disabled && value" class="reupload-img" @click.stop="mockClick()">重新上传</div>
...
<script>
import { getImgPreviewPath } from '@noah/tools/src/utils/util';
export default {
  props: {
    cropper: {
      type: Boolean,
