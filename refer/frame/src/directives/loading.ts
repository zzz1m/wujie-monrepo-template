...
type FnVoid = () => void;
const handleLazyFns: Record<string, FnVoid> = {}; // 懒加载，图片滚动到可视区域再加载
// 默认的加载中
const _loading =
  'https://img.bosszhipin.com/static/file/2021/5avahha8xo1638175251285.gif';
const errImg =
  'https://img.bosszhipin.com/static/file/2021/1wgkhwata61639388719175.jpg';
// 将html文本设置为img的src
function setImgErrHtml(el: HTMLElement, err_html: string) {
  const data = `
...
async function handleLoad(
  ...
  let placeholder = el.querySelector('.load_placeholder') as HTMLElement | null;
  if (tagName === 'canvas') {
...
function setCanvasLoading(
  ...
  const step = 2;
  const draw = (prev: number) => {
    // 如果该节点已不在页面，停止动画
    const { width, height } = canvas.getBoundingClientRect();
    if (
      !(width && height) ||
      el.attributes[_eid as unknown as number].value !== 'loading'
    ) {
...
function handleChange(el: HTMLElement, binding: any, vnode: any) {
  if (!binding.value) {
    return;
  }
  ...
  // 给节点一个唯一标识
  const ekey = Object.keys(el.attributes).find(
    (e) => el.attributes[e as unknown as number].name.indexOf('vload_') === 0,
  ...
  if (lazy) {
    // 处理懒加载，当节点滚动到可视区域再加载
    handleLazyFns[_eid] &&
      document.removeEventListener('scroll', handleLazyFns[_eid], true);
...
export default {
  ...
  unmounted(el: HTMLElement) {
    ...
    const _eid =
      (ekey && el.attributes[ekey] && el.attributes[ekey].name) || '';
    handleLazyFns[_eid] &&
      document.removeEventListener('scroll', handleLazyFns[_eid], true);
