<template>
<div v-clickoutsider="hide" 
    :class="['ui-tree-viewer', $attrs.clearable && !limited ? 'clearable' : '']"
>
    ...
    <ui-tooltip v-if="limited"
        :content="checkedName || label" placement="top" effect="dark"
    >
        <ui-select
            v-model="placeVal"
            ...
            @on-clear="placeClear"
        >
            <ui-option value="占位" label="占位">占位</ui-option>
        </ui-select>
    </ui-tooltip>
       
    <ui-select v-else ref="select"
        v-model="nodevalue" class="ctrl-select"
        :clearable="$attrs.clearable"
        :disabled="$attrs.disabled"
        :placeholder="$attrs.clearable ? placeholder : '请选择' " 
        @click.native="toggle" @on-clear="clearSelect"
    >
        <ui-option v-for="item in droplist" 
            :key="item.id" :value="item.id" :label="item.name"
        >{{ item.name }}</ui-option>
    </ui-select>
    <div class="tree-box">
        <ui-input v-if="show && filter" ref="filterInput"
            v-model="filterValue"
            type="text" placeholder="请输入筛选条件"
            @click.native="handleInputClick"
        ></ui-input>
        <ui-tree v-if="show" ref="tree" class="ctrl-tree"
            v-bind="$attrs" v-on="$listeners" @check-change="onCheckChange"
...
import { calcStrLength, debounce } from '@/utils/common';
...
export default {
    ...
    methods: {
        ...
        checkChange() {
            ...
            this.checkedName = nameList.join(this.labelJoin || ',');
