...
const openObserver = function( targetNode, cb ){
    // 选择将观察突变的节点
    targetNode = (
        targetNode && typeof targetNode === 'string' ? document.querySelector(targetNode) : targetNode
    ) || document.body;
    if( !window.MutationObserver ){
        ...
        targetNode.addEventListener('DOMSubtreeModified', targetNode.callback = function(e){
            cb && cb();
        });
    }else {
        cb && cb();
