...
import { offerRemindTaskCount } from '@/services/dashboard/entry';
import { _getUserInfo, _getModulePoster, _getResouces } from '@/services/dashboard/common';
import { _getAuditCount } from '@/services/dashboard/audit/index.js';

Vue.use(Vuex);

// const moduleCtx = require.context('./module', true, /\.m\.js$/);
// const moduleCfg = moduleCtx.keys().reduce((cur, file) => {
...
export default new Vuex.Store({
    namespaced: true,
    strict: import.meta.env.VITE_ENV=== 'development',
    modules: { ...moduleCfg },
    state: {
        token: '',
        companyId: '',
    ...
    actions: {
        ...
        async getTaskCount ({commit}) {
            try {
                const res = await offerRemindTaskCount();
                if (res.success) {
                    commit('updateTaskCount',res.root);
                }
            } catch(e) {
                console.log(e);
            }
