...
<!-- 表格区域 -->
<div class="data-list">
    <slot name="custom" :data="tableData" :isLoading="isLoading" />
    <ui-table-pro
        :data="tableData" class="table-pro-reset-ui"
        tooltip-effect="dark" :tooltip-max-width="tooltipMaxWidth"
    ...
    >
        ...
        <template v-if="columnList.length">
            <template v-for="(item, index) in columnList">
                <ui-table-column
                    ...
                    :min-width="item.minWidth" :width="item.width" :sortable="item.sortable"
                ...
                >
                    ...
                    <template slot-scope="{row}">
                        <component :is="item.customEl" v-if="item.customEl" :row="row" :custom-data="customData" />
                        <!-- 处理状态 -->
                        <Status v-else-if="item.stateKey" :show-state-map="item.showStateMap" :state-key="item.stateKey" :row="row" :state-name="item.render ? item.render(row) : getName(row, item)" />
                        <!-- 处理链接 path同按钮 为string或者function  -->
                        <a v-else-if="getPath(item, row).match(/^http/i)" :href="getPath(item, row)" :target="item.target || '_self'">{{ item.render ? item.render(row) : getName(row, item) }}</a>
                        <router-link v-else-if="getPath(item, row) && !getPath(item, row).match(/^http/i)" :to="getPath(item, row)" :target="item.target || '_self'">{{ item.render ? item.render(row) : getName(row, item) }}</router-link>
        ...
        <ui-table-column
            ...
            :fixed="operationFixed" class-name="operate"
            label="操作" :width="operationWidth"
        >
            <template slot-scope="{row, $index}">
                <btn-wrapper :btn-list="operationList" :max-show="operationBtnMax" :data="row" @op-click="(key, row) => handleOpClick(key, row, $index)"></btn-wrapper>
            </template>
        </ui-table-column>
    </ui-table-pro>
    <div v-if="!hiddenPager" v-show="!isHidePager" class="fasion-pager text-center" :class="pagerClass">
        ...
        <ui-pager
            ...
            :page-size="pageData.pageSize" :page-size-opts="pageSizeOpts"
...
import { _getBookList, _bookCancel } from '@/services/dashboard/pscc';
import { debounce } from '@/utils/common';
import { _getValue, getNameByCode, isNull, agentWarper } from '@/utils/util.js';
...
export default {
    ...
    created() {
        ...
        if (this.initOnSearch) { this.isLoading = true;}
        this.init();
    },
    mounted() {
    },
