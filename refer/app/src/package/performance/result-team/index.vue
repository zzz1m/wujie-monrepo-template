...
<div class="page-content flex-page-content flex-list-ui-page team-result page-performance" style="padding:20px;background:#F7F8FA">
    ...
    <ui-pagetip v-if="!inited" type="loading" style="background: transparent"></ui-pagetip>
    <div v-else-if="kpiKeyList.length" class="jixiao-content">
        <TabSelect v-show="kpiKeyList.length" :tabs="kpiKeyList" :active.sync="kpiKey" @change="onTabChange"></TabSelect>
        <div class="page-title no-preline" style="display: flex;align-items: center;padding-bottom:0;margin-top:0px">
            <div style="width:380px;height:32px;pointer-events: none;display:flex; align-items: center">
                <PageTitle>绩效结果</PageTitle>
            ...
            <div v-if="hasResult" style="flex:1;display:flex;justify-content: end;-webkit-justify-content: flex-end;">
                ...
                <div class="state-list-tab">
                    <div v-for="(item) in tabs" :key="item.value" :class="{ active: item.value === tabActive}" class="state-item" @click="(tabActive = item.value) && onSearch()">
                        {{ item.label }}
                    </div>
                ...
                <div class="fix-table-warp">
                    <div class="fix-table-content">
                        ...
                        >
                            ...
                            <ui-table-column prop="rltCalss" min-width="100" label="绩效级别" show-overflow-tooltip></ui-table-column>
                            <ui-table-column width="200" label="操作">
                                <template slot-scope="{row:item}">
                                    <router-link
                                        class="text-primary column-operate-btn"
                                        :to="`/dashboard/performance/evaluate-view-entry?userId=${item.userId}&twType=LEADER&kpiKey=${item.kpiKey}&type=admin&target=/dashboard/performance/result-team`"
                                    >
                                        查看评价
                                    </router-link>
                                </template>
...
import { teamkpi, _getKpiInfoList, _getDeptLeaderKpiTreeHistory, _getSchemeByTwType, _getStateByKpi } from '@/services/dashboard/performance';
...
export default {
    mixins: [kipDate, pathController],
    components: { TabSelect, partmentTree, TableEmptyTip, ClassDashboard, PageTitle, TimeRange,  EmptyPage /*jiaozhunDailog*/ },
    data() {
        return {
            hasResult: true,
            inited: false,
    ...
    async mounted() {
        ...
        this.getDeptList();
        await this.rawAdjustKpi();
        this.inited = true;
        this.deptId && this.onToggle({id: this.deptId, name: this.deptName});
    },
    computed: {
        dateStr() {
            const activeTab = this.kpiKeyList.find(t => t.kpiKey === this.kpiKey);
            if(!activeTab) return '';
            return ['startDate', 'endDate'].map(key => this.$dayjs(activeTab[key]).format('MM.DD')).join('-');
        },
        pageTitle () {
        ...
        bgStyle() {
            const tabIndex = this.tabs.findIndex(t => t.value === this.tabActive);
            const styles = [
                `width: ${ tabIndex? 66: 52}px`
            ];
            if(tabIndex > 0) {
                console.log(tabIndex, `transform:translateX(${ 54 + 66 * (tabIndex - 1) })px`);
                styles.push(`transform:translateX(${ 55 + 69 * (tabIndex - 1) }px)`);
            }
    ...
    methods: {
        ...
        async getSchemeByTwType() {
            ...
            if (success && root?.length) {
                ...
                if(root.find(t => t.kpiKey === this.$route.query.kpiKey)) {
                    this.kpiKey = this.$route.query.kpiKey;
                } else {
        ...
        async rawAdjustKpi() {
            // this.hasResult = await this.getState();
