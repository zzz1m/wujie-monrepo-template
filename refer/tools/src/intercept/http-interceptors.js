...
import { qs, filterQueryParams } from '../utils/util';
import { decrypt } from '../utils/crypto';
import magpie from 'magpie-monitor';
import { magpieConfig } from '../utils/monitor';
...
const handleResponseEnd = function (response) {
  // 喜鹊性能监控
 ...
 */
// 基座的地址，解决跨域问题
const wujieBase = process.env.NODE_ENV === 'development' ? 'http://localhost:5130/' : '/';
const instance = axios.create({
  baseURL: window.$wujie ? wujieBase : '/',
...
instance.interceptors.response.use(
  (response) => {
    ...
    if (data && data.callback) {
      //不能直接用后端返回的callback，因为server无法获取到当前浏览器访问的地址
    ...
    if (!decryptData.success && content !== '没有访问权限' && content !== '重复请求，请稍后再试' && isLogging <= 1 && !config.quiet) {
      // 自定义属性：quiet（控制是否toast）
      data.code != 0 &&
        content &&
        vue.$toast({
          // 提示错误信息
