 ...
 * @returns {String}
 */
export const getNameByCode = function (code, options = [], defaultValue = '', { codeKey = 'dictCode', labelKey = 'dictName' } = {}) {
 ...
 * @param {Number} speed  时间间隔
 * @returns {Array} 时间列表
 */
export function getTimeList(startTime, endTime, speed) {
  let currentDay = dayjs().format('YYYY-MM-DD');
  let start = +dayjs(`${currentDay} ${startTime}:00`);
  let end = +dayjs(`${currentDay} ${endTime}:00`);
  let item = '',
    time = '',
    list = [];
  let len = (end - start) / (speed * 60 * 1000);
  for (let i = 0; i <= len; i++) {
    if (i != 0) start += speed * 60 * 1000;
    time = dayjs(start).format('HH:mm:ss');
    item = dayjs(start).format('HH:mm');
    list.push({
      index: i,
      time: time,
      value: item,
      label: item,
      fullTimeValue: item + ':00'
    });
  }
  return list;
}
...
/**
 * @description 生成时间选择列表
 * @param {String} startTime 开始时间
 * @param {String} endTime 结束时间
 * @param {Number} speed  时间间隔
 * @returns {Array} 时间列表
 */
export function generateTimeList(startTime, endTime, speed) {
  let timeList = getTimeList(startTime, endTime, speed);
  let timeMapList = [];
  // 修改时间格式 增加结束时间 用于表格显示
  for (let i = 0; i < timeList.length - 1; i++) {
    let timeObj = timeList[i];
    timeObj.endTime = timeList[i + 1].value;
    timeObj.fullendTimeValue = timeList[i + 1].fullTimeValue;
    timeObj.timeRangStr = timeList[i].value + '-' + timeList[i + 1].value;
    timeMapList.push(timeObj);
  }
  return timeMapList;
}
